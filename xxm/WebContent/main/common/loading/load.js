...
(function (factory) {
	...
	$.mask_fullscreen = function(timeout){
		...
		var mask = '<div class="mask" ele="full_screen"><div>加载中...</div></div>';
		$("body").append(mask);
	...
	$.mask_element = function(ele_id, timeout){
		...
		//添加遮罩元素
		var mask = '<div class="mask" ele='+ele_id+' style="width: '+$(ele_id).width()+'px !important; height: '+$(ele_id).height()+'px !important; left: '+$(ele_id).offset().left+'px !important; top: '+$(ele_id).offset().top+'px !important;"><div>加载中...</div></div>';
		$("body").append(mask);
