...
var actionFormatter = function(value, row, index) {
    ...
    src2 = src.format(value,customerVO.customerId,row.taskDefinitionKey,row.taskName,row.taskId,"signInfo");
//    src3 = src.format(value,customerVO.customerId,row.taskDefinitionKey,row.taskName,row.taskId,"loanInfo");
//    src4 = src.format(value,customerVO.customerId,row.taskDefinitionKey,row.taskName,row.taskId,"mortgageinfo");
//    src5 = src.format(value,customerVO.customerId,row.taskDefinitionKey,row.taskName,row.taskId,"loanUpload");
//    src6 = src.format(value,customerVO.customerId,row.taskDefinitionKey,row.taskName,row.taskId,"mortgageUpload");
...
//    result += '<a class="btn btn-xs red J_menuItem" href="'+src4+'" title="抵押登记">抵押登记 <span class="glyphicon glyphicon-edit"></span></a>';
//    result += '<a class="btn btn-xs red J_menuItem" href="'+src5+'">放款影像补充<span class="glyphicon glyphicon-search"></span></a>';
//    result += '<a class="btn btn-xs red J_menuItem" href="'+src6+'">抵押影像补充<span class="glyphicon glyphicon-search"></span></a>';
...
//bootstrapTable初始化参数
var init = {
    url: '/xxm/rest/apply/applyService/findAllApplyInfoList', //请求后台的URL（*）
    method: 'post',                      //请求方式（*）
    ajaxOptions: {
        async: true // 设置为异步
    },
