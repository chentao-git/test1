...
//    initFormValid();
  ...
  //时间控件初始化
	laydate({elem : "#serviceTime",event : "focus"});
	//类型
	CommonUtils.findBaseDataOption("#processingProgressVL","PROCESSING_PROGRESS",1);
	CommonUtils.findBaseDataOption("#processingProgress","PROCESSING_PROGRESS",1);
    //添加按钮点击事件
    ...
    $("#btnDelete").click(function(){
        ...
        if(rows.length <=0 ){  
        	...
        	for(var i = 0; i < rows.length; i++){
        		ids.push(rows[i].serviceRecordId);
        	}
        	//批量删除用户信息
        	deleteUser(ids);
        }
...
var queryParams = function (params) {
    var temp = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
        ...
        processingProgressVL: $("#processingProgress").val(),//处理进度
        contentRegistration: $("#contentRegistration").val(),  //内容登记
...
var showModal =function(type,serviceRecordId){
	//重置表单
	...
	if(type == 2){
		...
		$("#basicDataForm").find(".has-error").removeClass("has-error");
		//查询用户详细信息
		var dataInfo = findDataInfo(serviceRecordId);
		//时间格式转化下为yy-mm-dd格式
		dataInfo.serviceTime = formatDate(dataInfo.serviceTime);
		//填充表单数据
		CommonUtils.fillFormData("#basicDataForm",JSON.stringify(dataInfo));
		
		$("#processingProgressVL option[value='"+dataInfo.processingProgressVL+"']").attr("selected","selected");
		
		//填充隐藏Id
		$("#basicDataForm").find("#modelId").val(serviceRecordId);
	}
	//详情
	if(type == 3){
...
var save=function(){
	...
	var url = null;
	if(type == "1"){//添加
		url = "/xxm/rest/customerFile/customerFileService/addServiceRecord";
		formData.baseCustomerId = baseCustomerId;
	}else{//修改
		var serviceRecordId = $("#basicDataForm").find("#modelId").val(); 
		formData.serviceRecordId = serviceRecordId;//设置用户id
		url = "/xxm/rest/customerFile/customerFileService/updateServiceRecord";
	}
	//请求后台
	$.ajax({
		url:url,
...
//查询详细信息
var findDataInfo=function(serviceRecordId) {
	var dataInfo = null;
	$.ajax({url:'/xxm/rest/customerFile/customerFileService/findServiceRecord',
		data:JSON.stringify({serviceRecordId:serviceRecordId}),
		type:'post',  
		dataType:'json',
		async:false,//同步请求
		contentType: "application/json",
