$(function () {
	//类型
	CommonUtils.findBaseDataOption("#followUpCategory","FOLLOW_UP_CATEGORY",1);
...
//    initFormValid();
  ...
  //时间控件初始化
	laydate({elem : "#followUpTime",event : "focus"});
...
var queryParams = function (params) {
    var temp = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
        ...
        followUpCategory: $("#follow").val(),//跟进类别
        contentRegistration: $("#contentRegistration").val(),  //跟进内容
    };
    return temp;
};
//操作
...
var initFormValid = function(){
	...
	var validate = $("#basicDataForm").validate({
		rules : {
			...
			followUpCategory: "required",
			contentRegistration : "required"
		},
		messages : {
			followUpTime : e + "请输入跟进时间",
			followUpCategory : e + "请输入跟进类别",
			contentRegistration : e + "请输入内容登记"
		},
...
var showModal =function(type,scheduleId){
	//重置表单
	...
	if(type == 2){
		...
		$("#basicDataForm").find(".has-error").removeClass("has-error");
		//查询用户详细信息
		var dataInfo = findDataInfo(scheduleId);
		//时间格式转化下为yy-mm-dd格式
		dataInfo.followUpTime = formatDate(dataInfo.followUpTime);
		//填充表单数据
		CommonUtils.fillFormData("#basicDataForm",JSON.stringify(dataInfo));
		
		$("#followUpCategory option[value='"+dataInfo.followUpCategory+"']").attr("selected","selected");
		
		//填充隐藏Id
		$("#basicDataForm").find("#modelId").val(scheduleId);
	}
	//详情
	if(type == 3){
...
var save=function(){
	...
	var url = null;
	if(type == "1"){//添加
		url = "/xxm/rest/customerFile/customerFileService/addCustomerSchedule";
		formData.baseCustomerId = baseCustomerId;
	}else{//修改
		var scheduleId = $("#basicDataForm").find("#modelId").val(); 
		formData.scheduleId = scheduleId;//设置用户id
		url = "/xxm/rest/customerFile/customerFileService/updateCustomerSchedule";
	}
	//请求后台
	$.ajax({
		url:url,
...
//查询用户详细信息
var findDataInfo=function(scheduleId) {
	var dataInfo = null;
	$.ajax({url:'/xxm/rest/customerFile/customerFileService/findCustomerSchedule',
...
//删除
var deleteUser =function(scheduleIds){
...
//		debugger;
		//记录删除用户的所有Id
		var param = null;
		//判断scheduleIds是不是数组类型
		if(scheduleIds instanceof Array){
			param = {
			...
			//定义一个数组
			var arr = new Array();
			arr.push(scheduleIds);
			param = {
					scheduleIds:arr
			};
		}
	    $.ajax({url:'/xxm/rest/customerFile/customerFileService/deleteCustomerSchedule',
