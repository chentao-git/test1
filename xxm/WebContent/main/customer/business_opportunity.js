...
//    initFormValid();
	//类型
	CommonUtils.findBaseDataOption("#focusOnProductVL","FOCUS_ON_PRODUCTS",1);
	CommonUtils.findBaseDataOption("#focusOnProduct","FOCUS_ON_PRODUCTS",1);
    //下拉控件初始化
    initSelect();
  ...
  //时间控件初始化
	laydate({elem : "#expectedTransactionTime",event : "focus"});
	laydate({elem : "#actualTransactionTime",event : "focus"});
    ...
    $("#btnDelete").click(function(){
        ...
        if(rows.length <=0 ){  
        	...
        	for(var i = 0; i < rows.length; i++){
        		ids.push(rows[i].businessOpportunityId);
        	}
        	//批量删除用户信息
        	deleteUser(ids);
        }
...
var queryParams = function (params) {
    var temp = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
        ...
        focusOnProductVL: $("#focusOnProduct").val(),//关注产品
        remark: $("#remark").val(),  //状态内容
...
var showModal =function(type,businessOpportunityId){
	//重置表单
	...
	//添加
	if(type == 1){
		$('#basicDataModal').find("h4").html("添加商机");
		//显示区域
		$("#edit").show();
		$("#detail").hide();
		//保存按钮显示
	...
	//修改
	if(type == 2){
		$('#basicDataModal').find("h4").html("修改商机");
		//显示区域
		$("#edit").show();
		$("#detail").hide();
		//保存按钮显示
		...
		$("#basicDataForm").find(".has-error").removeClass("has-error");
		//查询用户详细信息
		var dataInfo = findDataInfo(businessOpportunityId);
		//时间格式转化下为yy-mm-dd格式
		dataInfo.expectedTransactionTime = formatDate(dataInfo.expectedTransactionTime);
		dataInfo.actualTransactionTime = formatDate(dataInfo.actualTransactionTime);
		//填充表单数据
		CommonUtils.fillFormData("#basicDataForm",JSON.stringify(dataInfo));
	
		$("#focusOnProductVL option[value='"+dataInfo.focusOnProductVL+"']").attr("selected","selected");
		
		//填充隐藏Id
		$("#basicDataForm").find("#modelId").val(businessOpportunityId);
	}
	//详情
	if(type == 3){
		$('#basicDataModal').find("h4").html("商机详情");
		//查询用户详细信息
		var dataInfo = findDataInfo(businessOpportunityId);
		dataInfo.expectedTransactionTime = formatDate(dataInfo.expectedTransactionTime);
		dataInfo.actualTransactionTime = formatDate(dataInfo.actualTransactionTime);
...
var save=function(){
	...
	var loanAmount = formData.loanAmount;
	if(isNaN(loanAmount)){
		swal({title : "很抱歉",text : "请输入数字",type : "error"});
		return;
	}else if(isNaN(interestRate)){
		swal({title : "很抱歉",text : "请输入1-100的数字",type : "error"});
		return;
	...
	var url = null;
	if(type == "1"){//添加
		url = "/xxm/rest/customerFile/customerFileService/addBusinessOpportunity";
		formData.baseCustomerId = baseCustomerId;
	}else{//修改
		var businessOpportunityId = $("#basicDataForm").find("#modelId").val(); 
		formData.businessOpportunityId = businessOpportunityId;//设置用户id
		url = "/xxm/rest/customerFile/customerFileService/updateBusinessOpportunity";
	}
	//请求后台
	$.ajax({
		url:url,
...
//查询详细信息
var findDataInfo=function(businessOpportunityId) {
	var dataInfo = null;
	$.ajax({url:'/xxm/rest/customerFile/customerFileService/findBusinessOpportunity',
		data:JSON.stringify({businessOpportunityId:businessOpportunityId}),
		type:'post',  
		dataType:'json',
		async:false,//同步请求
		contentType: "application/json",
...
//		debugger;
	    ...
	    $.ajax({url:'/xxm/rest/customerFile/customerFileService/deleteBusinessOpportunity',
