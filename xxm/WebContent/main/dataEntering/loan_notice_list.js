$(function () {
    ...
    //初始化table并刷新数据
    $('#tbLoanNoitce').bootstrapTable(init);
    
    //查询Button的点击事件
    $("#btnQuery").click(function(){
    	//刷新列表
    	$("#tbLoanNoitce").bootstrapTable('refresh', init);
    });
...
//bootstrapTable初始化参数
var init = {
    url: '/xxm/rest/entering/enteringService/findLoanNoticeList', //请求后台的URL（*）
    method: 'post',                      //请求方式（*）
    ajaxOptions: {
        async: false // 设置为异步
    },
    ...
    uniqueId: "loanInfoInformId",                 //每一行的唯一标识，一般为主键列
    showToggle:false,                    //是否显示详细视图和列表视图的切换按钮
    cardView: false,                    //是否显示详细视图
    detailView: false,                   //是否显示父子表
	columns: [{
			 field:'loanInfoInformId',
             title: '操作',
             width: 100,
             align: 'center',
             valign: 'middle',
...
var showModal =function(type,loanInfoInformId){
	//重置表单
	...
	if(type == 3){
		...
		//查询用户详细信息
		findLoanNoticeInfo(loanInfoInformId);
		//显示区域
		$("#edit").hide();
		$("#detail").show();
		//保存按钮隐藏
...
//查询申请信息
function findLoanNoticeInfo(loanInfoInformId){
	var data={loanInfoInformId:loanInfoInformId};
	$.ajax({
		url:"/xxm/rest/entering/enteringService/findLoanNoticeInfo",
