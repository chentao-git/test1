...
var actionFormatter = function(value, row, index) {
    ...
    result += '<a class="btn btn-xs red J_menuItem" href="../main/dataEntering/project_data2.html?applyInfoId='+value+'&isEdit=1"><span class="glyphicon glyphicon-pencil"></span><label style="display:none;">修改</label></a>';
    result += '<a class="btn btn-xs red J_menuItem" href="../main/dataEntering/project_data2.html?applyInfoId='+value+'&isEdit=0"><span class="glyphicon glyphicon-search"></span><label style="display:none;">详情</label></a>';
    return result;
}
...
var init = {
	...
	columns: [{
			...
			field: 'policyinfoReturnVO.proposalNo',
			title: '投保单号',
			sortable : true,
			visible: false
		}, {
			field: 'policyinfoReturnVO.saveMessage',
			title: '响应信息',
			sortable : true,
			visible: false
		}
...
//查询申请信息
function findApplyInfo(applyInfoId){
	...
	$.ajax({
		...
		success:function(data) {
			if(data){
				$("#mktProdTypeName").text(data.mktProdTypeName);
				$("#cooprProdTypeName").text(data.cooprProdTypeName);
