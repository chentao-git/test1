...
import com.xxm.it.piic.service.IPICCUploadFileService;
...
import com.xxm.it.system.service.impl.UploadFileServiceImpl;
...
@Component
@Transactional
public class PICCUploadFileServiceImpl implements IPICCUploadFileService {
	...
	private String uploadAttachment(int fileSizeMax, int fileMinSize, String catalog,String attachmentType) throws Exception {
		...
		try {
			...
			String prejudicNo=request.getParameter("prejudicNo");//预审单号
//			String attachmentItem=request.getParameter("attachmentItem");
			String productionVl=request.getParameter("productionVl");//文件命名规范
			String stage=request.getParameter("stage");//阶段
			String attachmentItemID=request.getParameter("attachmentItemID");//id
			String attachmentItem=request.getParameter("attachmentItem");
			if (formItems != null && formItems.size() > 0) {
				// 存放附件Id字符
				String attrId = "";
...
//						String newFileName = System.currentTimeMillis() + fileName.substring(fileName.lastIndexOf("."));
						AttachmentTextVO attrVO = new AttachmentTextVO();
						attrVO.setExtendField1(stage);
						attrVO.setExtendField2(productionVl);
						attrVO.setAttachmentItemID(attachmentItemID);
						attrVO.setAttachmentItem(attachmentItem);
						List<AttachmentTextVO> AttachmentVOList=attachmentDao.findAttachments(attrVO);
						String newFileName="";
						if(AttachmentVOList!=null&&AttachmentVOList.size()>0){
							int str1=AttachmentVOList.size()+1;
							DecimalFormat df=new DecimalFormat("00");
							String str2=df.format(str1);
							newFileName = "A202009_"+certNo+productionVl+"_"+str2+ fileName.substring(fileName.lastIndexOf("."));
						}else{
							newFileName = "A202009_"+certNo+productionVl+ fileName.substring(fileName.lastIndexOf("."));
						}
//						String newFileName = "A202009_"+certNo+"_"+productionVl+ fileName.substring(fileName.lastIndexOf("."));
						filePath = uploadPath + File.separatorChar + newFileName;
						logger.info("文件重命名地址：" + filePath);
						// 保存文件到硬盘
						item.write(new File(filePath));
						...
						byte[] bs = item.get();
						String urls = XxmUtils.getBaseDataExtendField1("SERVICE_URL_CONFIG", "callFileStreamInterface");
						//初审：APPLY  放款：LOAN  车抵信息补充：LOANAFTER 非银行影像资料：PIC
						URL url = new URL(urls+"?filename="+newFileName+"&plateformcode=CPI000471&MktProdType="+stage+"&prejudicno="+prejudicNo);
						String returnStr = PICCServiceHandler.callFileStreamInterface(url, bs);
						if(!"".equals(returnStr)){
							//字符转Map
							Gson gson = new Gson();
							Map<String, Object> map = new HashMap<String, Object>();
							map = gson.fromJson(returnStr, map.getClass());
							System.out.println("returnMap:" + map.get("bucket"));
							System.out.println("returnStr:" + returnStr);
							if("00".equals(map.get("hash"))){//接口上传成功
								// 保存附件信息
								...
								attachmentTextVO.setAttachmentStatus(BaseDataConstant.STATUS_INVALID);//失效
								attachmentTextVO.setExtendField1(stage);//大类
								attachmentTextVO.setExtendField2(productionVl);//小类
								...
								msgMap.put("remark", map.get("bucket"));//错误信息
							}
						}else{
							msgMap.put("remark", "接口异常！");//错误信息
						}
					}
				}
			...
			if (msgMap.get("attrId") != null&&!"".equals(msgMap.get("attrId"))) {
				msgMap.put("flag", true);
				msgMap.put("msg", "上传文件成功！");
			} else {
				msgMap.put("flag", false);
				msgMap.put("msg", msgMap.get("remark"));
			}
		} catch (Exception ex) {
			msgMap.put("flag", false);
			msgMap.put("msg", "上传文件失败，请重试！ERROR INFO:" + ex.getMessage());
